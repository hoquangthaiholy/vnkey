...
struct ConvertLookupKey {
    Uint32 charCode;
    Uint8 code;
    bool operator==(const ConvertLookupKey& other) const {
        return charCode == other.charCode && code == other.code;
    }
};

struct ConvertLookupKeyHash {
    std::size_t operator()(const ConvertLookupKey& k) const {
        return std::hash<Uint32>()(k.charCode) ^ (std::hash<Uint8>()(k.code) << 8);
    }
};

static std::unordered_map<ConvertLookupKey, std::pair<int, int>, ConvertLookupKeyHash> convertLookupMap;
static bool convertLookupMapInited = false;

static void initConvertLookupMap() {
    ...
    convertLookupMapInited = true;
}

static bool findKeyCode(const Uint32& charCode, const Uint8& code, int& j, int& k) {
    if (code >= 5) return false;
    if (!convertLookupMapInited) {
        initConvertLookupMap();
    }
    ConvertLookupKey key = { charCode, code };
    auto it = convertLookupMap.find(key);
    if (it != convertLookupMap.end()) {
        j = it->second.first;
        k = it->second.second;
        return true;
    }
